...
void MIDI(uint8_t Command, uint8_t Note)
{
	...
	case NOTE_ON:
		...
		HWREG(DEBUGGER_TX) = NOTE_ON;		// Command
		HAL_Delay(1);

		HWREG(DEBUGGER_TX) = Note;			// Note
		HAL_Delay(1);

		HWREG(DEBUGGER_TX) = 126;			// Velocity
		HAL_Delay(1);
		break;
	...
	case NOTE_OFF:
		...
		HWREG(DEBUGGER_TX) = NOTE_OFF;		// Command
		HAL_Delay(1);

		HWREG(DEBUGGER_TX) = Note;			// Note
		HAL_Delay(1);

		HWREG(DEBUGGER_TX) = 0x00;			// Velocity
		HAL_Delay(1);
		break;
	}
	DEBUGPRINT("\n");
