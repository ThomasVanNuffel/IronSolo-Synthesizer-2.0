...
void CheckScreen()
{
	...
	if(!Touch && TS_State.touchDetected)						// If the screen is newly pressed
	{
		...
		case DEN_TEST:
			...
			{
				if(TS_State.touchX[0] > (208 + i*21) && TS_State.touchX[0] < (229 + i*21) && TS_State.touchY[0] > 48 && TS_State.touchY[0] < 142)	// Keyboard 1
				{
					NoteOn(i + 12 * Octave_Amount + Key_Amount - 127, Indicators);
				}
			}

			for(uint8_t i = 0; i < 13; i++)
			{
				if(TS_State.touchX[0] > (208 + i*21) && TS_State.touchX[0] < (229 + i*21) && TS_State.touchY[0] > 168 && TS_State.touchY[0] < 267)	// Keyboard 2
				{
					NoteOn(i + 12 * Octave_Amount + 12 + Key_Amount - 127, Indicators);
				}
			}

			if(TS_State.touchX[0] > 209 && TS_State.touchX[0] < 282 && TS_State.touchY[0] > 0 && TS_State.touchY[0] < 45)	// -12
			{
				Octave_Amount--;
			}

			if(TS_State.touchX[0] > 282 && TS_State.touchX[0] < 345 && TS_State.touchY[0] > 0 && TS_State.touchY[0] < 45)	// -1
			{
				Key_Amount--;
			}

			if(TS_State.touchX[0] > 345 && TS_State.touchX[0] < 406 && TS_State.touchY[0] > 0 && TS_State.touchY[0] < 45)	// +1
			{
				Key_Amount++;
			}

			if(TS_State.touchX[0] > 406 && TS_State.touchX[0] < 480 && TS_State.touchY[0] > 0 && TS_State.touchY[0] < 45)	// +12
			{
				Octave_Amount++;
			}


			DEBUGPRINT("Octaves: +%d", Octave_Amount);
			DEBUGPRINT("Keys: +%d - 127", Key_Amount);
