...
void NoteOn(uint8_t Note, uint8_t Status)
{
	// MIDI
		...
		{
			DEBUGPRINT("MIDI staat uitgeschakeld");
		}

		// DDS
		if(Status & 2)						// If DDS bit is 1
		{
			DEBUGPRINT("DDS aan");
			DDS(NOTE_ON, Note);
		}
		...
		{
			DEBUGPRINT("DDSes zijn uitgeschakeld");
		}

		// LEDs
		if(Status & 1)						// If LED bit is 1
		{
			DEBUGPRINT("LEDs aan");
		...
		{
			DEBUGPRINT("LEDs zijn uitgeschakeld");
		}
...
void NoteOff(uint8_t Note, uint8_t Status)
{
	...
	if(Status & 2)						// If DDS bit is 1
	{
		DEBUGPRINT("DDS aan");
		DDS(NOTE_OFF, Note);
		DDS(SEND, STOP_PLAYING);		// Stop the envelope for the playing note
	}
	else									// If bit is 0
	{
		DEBUGPRINT("DDSes zijn uitgeschakeld");
